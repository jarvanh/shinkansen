...
test('對照只出現一次：writer 後處理首次保留、後續替換（keepTarget / keepSource）', async ({ context, extensionId }) => {
  ...
  const out = await page.evaluate(async () => {
    ...
    blocks[3].translationRaw = '眾人跟著甲（Alpha）前行。';
    blocks[3].translation = blocks[3].translationRaw;
...
test('一致性掃描：漂移偵測 → 套用統一譯名 + 術語表回填；術語表符合度違規列出', async ({ context, extensionId }) => {
  const page = await openDocPage(context, extensionId, {
  ...
  // 符合度違規：Utterson 分組列出（含指定譯名）
  const compRows = page.locator('#scan-compliance-list .scan-compliance-row');
  expect(await compRows.count()).toBe(1);
  const compText = await compRows.first().textContent();
  expect(compText).toContain('Utterson');
  expect(compText).toContain('厄特森');

  // 套用 → 全書不再有普勒、被改段落走 editedHtml（手動編輯語意）
  await cases.first().locator('button').click();
  await page.waitForTimeout(200);
  ...
  // 術語表回填 Poole→普爾 且 session 落地（IndexedDB 輪詢）
  start = Date.now();
  let sessionGloss = null;
  while (Date.now() - start < 5_000) {
    sessionGloss = await page.evaluate(() => new Promise((resolve) => {
      const req = indexedDB.open('shinkansen-epub-sessions');
      req.onsuccess = () => {
        const get = req.result.transaction('sessions').objectStore('sessions').getAll();
  ...
  // 入口按鈕消失，不留指向被取代譯文的過期發現
  const dlPromise = page.waitForEvent('download');
  await page.click('#chapters-export-session-btn');
  const dl = await dlPromise;
  await page.setInputFiles('#epub-session-import-file', await dl.path());
  let scanBtnHidden = false;
  start = Date.now();
  while (Date.now() - start < 5_000) {
    scanBtnHidden = await page.locator('#chapters-scan-btn').isHidden();
    if (scanBtnHidden) break;
    await page.waitForTimeout(100);
  }
  expect(scanBtnHidden).toBe(true);
  await page.close();
});

test('一致性掃描 option 關閉：翻譯完成後不掃描、不送 SCAN 訊息', async ({ context, extensionId }) => {
  const page = await openDocPage(context, extensionId, {
    drift: { term: 'Poole', renderings: ['普爾', '普勒'] },
    scanKnown: ['普爾', '普勒'],
  });
  ...
  expect(await page.locator('#chapters-scan-btn').isHidden()).toBe(true);
  await page.close();
});

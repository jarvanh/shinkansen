...
//      test (a)「只加 runtime-ios 不 zoom」fail(computed zoom 1.5);還原 → pass。
...
test('只加 runtime-ios（iOS build 跑在 Mac）：popup 不 zoom，但 PDF 入口仍隱藏', async ({ context, extensionId }) => {
  ...
  await page.setViewportSize({ width: 440, height: 956 }); // 寬 viewport,若誤套放大會是 1.435
...
test('加 runtime-ios-touch（真觸控裝置）：popup 放大到 1.435（寬 viewport）', async ({ context, extensionId }) => {
  const page = await context.newPage();
  ...
  expect(parseFloat(zoom)).toBeCloseTo(1.435, 3);
});
...
test('source 結構：放大掛 runtime-ios-touch、PDF 隱藏掛 runtime-ios、gate 在 isTouchScreenDevice', async ({ context, extensionId }) => {
  ...
  // 放大規則掛 runtime-ios-touch
  expect(popupCss).toMatch(/body\.runtime-ios-touch\s*\{[^}]*zoom:\s*1\.5/);

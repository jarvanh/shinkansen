...
- **雙翻譯引擎 + 自訂 Provider**：Gemini（AI 翻譯、品質最佳、需 API Key）+ Google Translate（非官方免費端點、不需 API Key、速度更快）+ 自訂 OpenAI-compatible Provider（v1.5.7 起，可接 OpenRouter / Together / Ollama 等百種模型），依場景自由切換
...
v1.5.7 起，除了 Gemini 與 Google Translate 兩條既有引擎，你還可以接一組 OpenAI-compatible 端點，使用 Gemini 之外的模型——例如：
...
2. 填三個必填欄位：
   ...
   - **API Key**：對應 provider 的 Bearer token
3. 選填：System Prompt（留空 = 用內建簡短預設）/ Temperature / 計價（USD per 1M tokens，自填，0 = 不顯示費用）
4. 儲存
5. 到「一般設定」分頁的「翻譯快速鍵」section，把任一組 preset 的引擎改為「自訂 Provider（OpenAI-compatible）」
6. 對該 preset 的快速鍵翻譯時就會走自訂 Provider

### 設計重點

- **System Prompt 獨立**：自訂 Provider 用獨立的 base prompt，不繼承 Gemini 分頁的設定
- **黑名單與固定術語表共用**：「禁用詞清單」與「固定術語表」分頁的設定會自動注入到 system prompt 末端，自訂 Provider 也享有，改一處兩邊同步生效
- **快取分區**：cache key 自帶 base URL hash，不同 provider 的同 model name 不會互相污染
...
- 目前只能設定**一組**自訂 Provider（如果未來需要 3 組 preset 各綁不同 provider，再擴）
- 必須是真正 OpenAI-compatible（`POST /chat/completions` + `Bearer` Authorization + 標準 `messages` 結構 + `usage.prompt_tokens` / `completion_tokens` 欄位）。Anthropic 與 Gemini 的**原生** API 不能直接接，但透過 OpenRouter 中轉就可以

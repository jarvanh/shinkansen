...
- **三語翻譯目標 + 三語介面**（v1.8.59 起 / v1.8.60 介面跟切）：可選翻譯成繁體中文（台灣）/ 簡體中文（中國）/ 英文。設定頁「翻譯目標語言」分頁切換,所有翻譯模式（網頁 / PDF / YouTube 字幕）共用此設定;v1.8.60 起擴充功能介面（popup / 設定頁 / 翻譯進度通知）會跟著切到對應語言（其他 5 個 target 走英文 fallback）。預設依瀏覽器語言推導
...
v1.8.61 — 完整功能清單與規格詳見 [SPEC.md](SPEC.md)。

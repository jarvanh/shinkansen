...
- **PDF 翻譯放寬上限並強化版面**：檔案上限放寬到 50 MB / 300 頁、可指定只翻其中幾頁；以 300 多份公開測試 PDF 建立驗證管道，修正雙欄短行、彩色底、旋轉頁、段落溢出等版面與渲染問題；簡中 / 日 / 韓譯文改用對應字型不再出現方框
- **省 token**：網頁翻譯每批容量翻倍（API 請求數減半、input token 省約三成，速度不變）；YouTube 自動字幕 AI 分句改用精簡傳輸格式，費用約減半
- **大量翻譯品質與穩定性修正**：網域專用術語表正確套用、譯文編輯模式強化、譯文被前端框架打回原文的守護補強、簡繁自動轉換後第一下翻譯不再被當成還原等

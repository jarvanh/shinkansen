...
| iOS／iPadOS Safari | 🚧 | TestFlight 階段（未上架）；四指輕點觸發（= 主要預設快速鍵完整 toggle，`content-touch.js`）；popup／options iOS 調整（popup viewport 依機型動態放大撐滿、popup 快速鍵提示改顯示四指輕點、options 窄版 RWD＋16px 輸入框防 focus 自動 zoom、隱藏 PDF 入口）；build 期 strip `translate-doc/`（不做 PDF 翻譯）；m.youtube.com 字幕翻譯完整支援（行動版 Safari 不需切電腦版） |
...
- **引擎**：Instapaper Full API（OAuth 1.0a + xAuth），`lib/instapaper.js`。走 Full API（非 Simple `/api/add`）是因為要送整頁 HTML 給 `bookmarks/add` 的 `content` 參數，由 Instapaper 端 readability 抽正文 → 存進去的是譯文版文章；Simple API 只吃網址會讓 Instapaper 重抓原文。
...
- **訊息協定**：
  - content `EXTRACT_PAGE_HTML` → 回 `{ ok, url, title, html }`（`SK.extractPageHtml`：clone document、剝除 `script/style/noscript/template/link`、媒體嵌入 `iframe/video/audio/object/embed`、擴充注入 UI `#shinkansen-toast-host`/`#shinkansen-dual-style`、保留 `<shinkansen-translation>` wrapper、`outerHTML` 序列化）。剝媒體嵌入是因為下游 reader 的正文抽取器會把影片嵌入升級成主要內容（實測 Instapaper 對含 Brightcove iframe 的文章頁抽到播放器 UI、丟掉整篇正文）。
...
- **回歸測試**：`test/unit/instapaper-oauth.spec.js`（RFC 5849 §3.4.1 base string 向量 + OAuth 1.0 A.5.2 HMAC-SHA1 簽章向量 + payload / parse / mock fetch 分支）、`test/regression/extract-page-html.spec.js`（擷取剝除 / 保留斷言）。

> OAuth 簽章細節、為何不前端跑 Readability、human review 卡關等設計脈絡見 SPEC-PRIVATE。

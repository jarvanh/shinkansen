...
| iOS／iPadOS Safari | 🚧 | TestFlight 階段（未上架）；四指輕點觸發（= 主要預設快速鍵完整 toggle，`content-touch.js`）；popup／options iOS 適配（popup viewport 依機型動態放大撐滿、popup 快速鍵提示改顯示四指輕點、options 窄版 RWD＋16px 輸入框防 focus 自動 zoom、隱藏 PDF 入口）；build 期 strip `translate-doc/`（不做 PDF 翻譯）；m.youtube.com 開字幕翻譯時提示切換電腦版 |

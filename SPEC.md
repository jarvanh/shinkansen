...
- 譯文接收後處理（v2.0.53 起，`repairDocLlmArtifacts` + `alignTrailingPeriodWithSource`，批次寫回 / 單段重試 / session 載入三接收點共用）：畸形佔位符修復（`⟦/2»` → `⟦/2⟧`）→ 段尾批次分隔符殘片清理 → 標記周邊與 CJK 內文 ASCII 空格收斂（全形空格 / 中英空格 / 換行不動）→ 句尾句號對齊原文（原文句尾無終止標點時刪掉譯文自補的「。」，只刪不加、手動編輯不碰）。快取命中也走同一條，舊壞快取自動治癒

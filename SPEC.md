...
| 懸浮按鈕 | ✅ | 頁面邊緣可拖移方形「新」icon（`content-floating-icon.js`）；短按走 `popupButtonSlot` 預設翻譯、長按選三組 preset 或「功能選單」（在頁內用 closed Shadow DOM + iframe 載入 `popup/popup.html?panel=1` 當浮層叫出工具列圖示選單，`popup.html` 列入 `web_accessible_resources`；popup 端偵測 `?panel=1` → 關閉改 postMessage 收浮層、並回報內容高度讓 iframe 收緊）、拖移吸附左右緣（`floatingIconPos` 位置記憶）；觸控裝置渲染時把按鈕夾離螢幕上下角落（避開 iPadOS 視窗縮放拖曳角，防止停太靠近角落被 OS 攔走觸控而拖不出來），預設右下角即落在此安全極限；手機／平板預設開、桌面預設關（`floatingIcon`）；可調透明度（`floatingIconOpacity`）；視覺 16px／可點 32px；closed Shadow DOM，不注入文章內容 |

...
- **翻譯品質**:預設啟用「術語表一致化」(`translateDoc.applyGlossary`)
...
**settings schema**:`settings.translateDoc = { systemPrompt, applyGlossary, temperature }`(top-level group)。為將來各 Office 格式擴充預留 `translateDoc.docx / .xlsx / .pptx` sub-key 空間 — `systemPrompt` / `temperature` 通用(inline marker 協定相同、生成參數一致),sub-key 只放格式特別設定(例 Excel 表格 cell 翻譯邏輯)。

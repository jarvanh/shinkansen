...
| 功能區塊 | 優先度 | 簡述 |
|---------|--------|------|
| **PDF 文件翻譯** | **高** | 擴充頁面（`translate-doc.html`）新分頁開啟；本機上傳 PDF（硬上限 50 頁 / 10MB，軟警告 30 頁 / 5MB）；純前端 PDF.js 抽 text run + bbox + font 重建版面 IR；段落為翻譯單位、公式 / 表格 / 圖片走 placeholder 不翻；沿用既有 preset 選翻譯引擎；線上閱讀器雙頁並排對照（原 / 譯）+ 雙向 scroll sync;pdf-lib 重新 render 對照 PDF 提供下載；翻譯失敗段落保留原文；完整規格見 §17 |
...
**強化段序號標記 `useStrongSegMarker`（預設 `true`）**：自訂 Provider 多段批次時，每段開頭加「<<<SHINKANSEN_SEG-N>>>」STRONG 格式序號標記，弱模型（如 gemma-4 量化版等本機量化 LLM）不會把它當自然語言誤譯為「N1、N2」洩漏到譯文。代價是每段批次多約 7 tokens（input 加 output 雙倍開銷）。商用 LLM 使用者（OpenRouter / DeepSeek / Groq 等）可在「自訂 Provider」分頁關閉此 toggle 改用緊湊「«N»」省 token。Gemini 主路徑不受此選項影響——固定使用「«N»」COMPACT。

**`customProvider.model` 為空的行為**:`lib/openai-compat.js`（translateChunk / extractGlossary）在 model 為空字串時**不送** `body.model` 欄位,讓 server 用啟動時鎖定的 model;對應 llama.cpp / Ollama 等本機 server 沒指定 model ID 的場景。商用後端（OpenAI / OpenRouter / DeepSeek）漏填會自然回 4xx「model required」,讓 provider error 自己講話。`background.js handleTranslateCustom` / `handleExtractGlossaryCustomProvider` 對齊此行為,**不**在前面提早擋空 model;`baseUrl` 仍是必填（連 endpoint 都沒有沒辦法呼叫）。
...
**設定**：`settings.targetLanguage`（合法值 8 個，見 `TARGET_LANGUAGES` 陣列），存 `chrome.storage.sync`，使用者可在 Options「翻譯目標語言」分頁切換。
...
- ✅ 來源語言偵測 target-aware
- ✅ Cache key 區隔
- ✅ Extension UI 字串 i18n（zh-TW / zh-CN / en 三語 UI）— v1.8.60 P2 落地，詳見 §3.10
- ⏳ 商店素材英文 listing — 留 P3

### 3.10 UI Localization（i18n，P2 / v1.8.60）

擴充功能 UI 字串支援三語（zh-TW / zh-CN / en）。UI 語言由獨立的 `settings.uiLanguage` 偏好控制，**跟翻譯目標 `settings.targetLanguage` 解耦**——可以「英文介面 + 翻譯目標繁中」或「繁中介面 + 翻譯目標英文」等任意組合。

**`settings.uiLanguage` 合法值**：

| 值 | 行為 |
|---|---|
| `'auto'`（預設） | 由 `resolveUiLanguage(navigator.language)` 推導：`zh-TW` / `zh-Hant` / `zh-HK` 系 → `zh-TW`；其他 `zh-*` → `zh-CN`；其他 → `en` |
| `'zh-TW'` / `'zh-CN'` / `'en'` | 強制鎖到該語言，不受 `navigator.language` / `targetLanguage` 影響 |

UI dict 永遠是三語（`messages_zhTW` / `messages_zhCN` / `messages_en`），不像 target 有 8 種；任何不在三語內的偏好（含 `'auto'` 推導為 `ja` / `ko` 等情境，或 5 語 target 經 fallback 鏈）最終都走 `en`。原 v1.8.60 P2 第一版的「UI 跟著 target 切」設計已撤回，因為使用者可能想用 en 介面但翻成繁中（或反之），雙設定獨立。

**自製 dict 而非 `chrome.i18n`**：Chrome 原生 `chrome.i18n` 綁瀏覽器 locale，無法跟 target 連動（使用者瀏覽器 zh-TW、把翻譯目標切 en，UI 仍會跟瀏覽器走繁中），故走自製 dict。`_locales/{en,zh_CN}/messages.json` 仍維持 Chrome Web Store / AMO 商店 listing 用的 `extName` / `extDescription` 兩條最小集。
...
每組 dict 約 473 條 entry，key 對齊；t() 內三層 fallback：`[TABLES[lang], TABLES[FALLBACK_LANG], TABLES['zh-TW']]`，`FALLBACK_LANG = 'en'`，最終 fallback 為 zh-TW（避免任何 key 缺漏導致 UI 顯示空字串）。
...
- `subscribeUiLanguageChange(cb)`：訂閱 `chrome.storage.onChanged` 對 `targetLanguage` 的變動，觸發 reapply
...
> 規劃中（尚未實作）。本章節為完整規格，規劃實作週期約 6-10 週。
...
- 譯文長度溢出 bbox 時：不裁切、不縮字級，允許溢出到 bbox 下方（下一個 block 的位置可能被覆蓋，屬已知限制，進 §17.10）
...
### 17.10 已知限制（進 SPEC 為防範未來踩坑）

1. **多欄論文跨欄段落**：雙欄論文中橫跨兩欄的圖 / 表 caption，版面演算法可能切錯欄位 → 偵測啟發式給 caption 加「橫跨多欄」flag，該 block 翻譯後在譯文 PDF 也跨欄 render
2. **複雜表格內文翻譯**：`table` block 預設不翻，但讀者可能想看表格內中譯——MVP 不做，進未來迭代清單
3. **公式中的英文變數說明**：公式 block 內若混合公式 + 英文變數說明文字（典型：「where x is the input variable」），目前整段不翻 → 接受此限制，使用者可手動複製送翻
4. **譯文長度溢出 bbox**：中譯文比英文短（平均 0.6-0.8 倍字寬），通常不溢出；但極短英文段（如 "Yes."）譯成中文後若需詞綴可能微溢——不裁切、不縮字、允許溢出
5. **文字型 + 影像混合 PDF**：若部分頁是文字、部分頁是掃描影像，文字頁正常處理，掃描頁直接 clone 原樣到譯文 PDF（不嘗試 OCR）

### 17.11 規劃中的測試覆蓋

- `test/regression/pdf/`：文件翻譯專用 regression 子目錄
- 每類 block 偵測各一條 fixture spec(`detect-paragraph` / `detect-heading` / `detect-list` / `detect-formula` / `detect-table` / `detect-caption` / `detect-footnote`)
- Column 偵測 spec（單欄 / 雙欄 / 三欄各一）
- Reading order spec（雙欄論文跨欄、含 footnote / page number 排序）
- 翻譯失敗 fallback spec（模擬 API 錯誤，確認 UI 顯示原文 + retry 按鈕）
- 下載譯文 PDF spec（產生實際 PDF 檔案，用 PDF.js 讀回確認結構）
- Scroll sync spec（模擬左欄 scroll，確認右欄對應 block 進入 viewport）

每條 spec 用最小重現 fixture（2-5 頁的人工合成 PDF），不依賴真實學術論文。

### 17.12 W7:inline rich text 樣式保留

W7 起譯文 PDF 保留原文的 **bold / italic / link** inline 樣式,從 W6 之前 block-level 一視同仁升級成 run-level。

#### 17.12.1 偵測層

- `pdf-engine.js parsePdf` 對每個 textRun 抽出 `isBold` / `isItalic` / `linkUrl`
  - bold:`font.bold === true || /Bold|Black|Heavy|Demi|Semi/i.test(name)`（name 從 commonObjs 拿）
  - italic:`font.italic === true || /Italic|Oblique|-It\b|-Obl\b/i.test(name)`
  - linkUrl:從 `page.getAnnotations()` 拿 Link list,run bbox 中心點落入 link rect → 該 link 的 url
- `layout-analyzer.js buildBlockFromLines` 把 runs 合併成 `block.styleSegments`(連續同 `(isBold, isItalic, linkUrl)` tuple 合一段)+ `block.linkUrls`（去重保 order 的 url 表）

#### 17.12.2 inline marker 協定

譯文送 LLM 時插 marker（沿用既有段落佔位符 `⟦…⟧` 格式）:

```
⟦b⟧粗體段⟦/b⟧
⟦i⟧斜體段⟦/i⟧
⟦l:N⟧連結文字⟦/l⟧   N = block.linkUrls 1-based index
```

巢狀順序（由外到內）:**bold → italic → link**。link 在最內層的理由:link 是錨點,跨樣式邊界比連續性重要;bold/italic 純樣式可被 wrap 切散。

`translate-doc/translate.js` 的 `buildMarkedText(block)` 從 styleSegments 構,送 LLM;譯文回來後 `parseMarkedTranslation(text, linkUrls)` stack-based parse 出 `block.translationSegments`。**parser 失敗策略**:tag 不成對 / 巢狀錯誤 / link index 越界 → 整 block 退回 plain regular（translationSegments 為單一 piece）,不破渲染。

#### 17.12.3 文件翻譯獨立 system prompt + 獨立 settings page

W7 起文件翻譯路徑用獨立 system prompt,**通用於 PDF 與未來各 Office 格式**(.docx / .xlsx / .pptx),不共用網頁 `DEFAULT_SYSTEM_PROMPT`。

**Prompt 拆兩段**（關鍵設計）:
- **`DEFAULT_DOC_SYSTEM_PROMPT`**(user-editable)= 跟網頁翻譯同款 `DEFAULT_SYSTEM_PROMPT`,使用者可在 settings page 調翻譯風格 / 用語規則。
- **`DOC_INLINE_MARKER_INSTRUCTION`**（內部常數,**user 看不到也改不到**）= `<inline_style_markers>` 區塊,教 LLM 處理 ⟦b⟧/⟦i⟧/⟦l:N⟧ 規則:
  1. 標記必成對（⟦b⟧ 對 ⟦/b⟧、⟦i⟧ 對 ⟦/i⟧、⟦l:N⟧ 對 ⟦/l⟧）
  2. 標記內中文字界可前後微調幾字以對應正確語意
  3. 連結編號 N 不可變動
  4. 標記可巢狀但不可交叉
  5. 原文無標記時譯文也不要無中生有加

`background.js TRANSLATE_DOC_BATCH` handler 把 user prompt + `DOC_INLINE_MARKER_INSTRUCTION` 拼接成 effective prompt,透過 `geminiOverrides.systemInstruction` 送 LLM。即使 user 把 systemPrompt 改成完全不同內容,marker 規則仍生效。

**為什麼隱藏 marker 區塊**:這段是「實作細節 / 知識權重」,不是使用者該調整的範疇 — 改錯會破壞 marker 解析核心邏輯;而且 inline marker 協定屬內部 know-how,不適合露出在 UI。

**獨立 settings page**:`translate-doc/settings.html`(+ `settings.js`),從 reader 工具列「翻譯設定」modal 內「進階設定 →」按鈕用 `chrome.tabs.create` 開新分頁進入。內容章節:
- **翻譯品質**:術語表一致化 （`translateDoc.applyGlossary`,預設關） + 套用固定術語表 （`translateDoc.applyFixedGlossary`,預設開,共用 `settings.fixedGlossary.global`）
- **翻譯 Prompt**:textarea + 重置按鈕
- **進階**:翻譯創意度 temperature（`translateDoc.temperature`,範圍 0–2,**預設 0.5**;合約 / 技術文件偏保守） + 清除所有文件翻譯快取（只清 `tc_*_doc*` entries,不影響網頁 / 字幕）

**settings schema**:`settings.translateDoc = { systemPrompt, applyGlossary, applyFixedGlossary, temperature }`(top-level group)。為將來各 Office 格式擴充預留 `translateDoc.docx / .xlsx / .pptx` sub-key 空間 — `systemPrompt` / `temperature` 通用（inline marker 協定相同、生成參數一致）,sub-key 只放格式特別設定（例 Excel 表格 cell 翻譯邏輯）。

**固定術語表共用**:`applyFixedGlossary` 只是「文件翻譯路徑要不要套用」開關;術語清單本身存於 `settings.fixedGlossary.global`,跟網頁翻譯共用同一份（術語表分頁編輯）。`background.js TRANSLATE_DOC_BATCH` 讀 `td.applyFixedGlossary !== false` 後傳給 `handleTranslate(payload, sender, overrides, null, '_doc', applyFixedGlossary)` 第 6 參數,沿用主翻譯的 fixedGlossary 注入邏輯（`buildEffectiveSystemInstruction`）。關掉後 cache key 因 fixedGlossary entries 從 prompt 移除而換新 hash(`_g<newhash>`),不主動清舊快取。

**Cache key 含 temperature**（只 `_doc` 路徑）:`background.js handleTranslate` cache key suffix 對 `cacheTag === '_doc'` 加 `_t<temp.toFixed(2)>`,讓使用者改 temperature 後立即生效（不會 cache hit 拿到舊 temp 譯文）。網頁 / 字幕路徑無獨立 temperature 設定不加,避免 cache 不必要分裂。

`background.js TRANSLATE_DOC_BATCH` 把 `translateDoc.temperature` 透過 `geminiOverrides.temperature` 注入,覆蓋全域 `geminiConfig.temperature` — 文件翻譯獨立 temperature,不影響網頁翻譯設定。

#### 17.12.4 渲染層

`pdf-renderer.js drawTranslatedOverlay` 走 piece-by-piece:

- **bold**:`page.drawText(piece.text, { font: cjkFontBold })`
- **italic**:`page.drawText(piece.text, { matrix: [1, 0, tan(12°), 1, cx, cy] })`
  - 12° skew transform 是業界標準折衷（CSS font-synthesis-style 預設 14°、FontForge -10° ~ -15°）
  - **不 vendor italic 字型**（節省 ~6MB 包大小）
  - 對 ASCII 字符視覺等同真 italic,中文字符也跟著傾斜（中文無真 italic 字型,業界一致行為）
- **link**:藍色 `rgb(0, 0.27, 0.55)` + `page.drawLine` underline（baseline 下 fontSize × 0.12 處）+ 收集譯文 device rect
- **link annotation click rect**:`addLinkAnnotations` 改用譯文 piece 的 device rect（原 PDF rect 對不到譯文位置）。原 PDF link 沒對應到任何譯文 piece 時（non-translatable 區 / translation 失敗）fallback 用原 rect 保留 click hit area

`wrapSegmentsToWidth` 取代既有 `wrapTextToWidth`,segment-aware:對每 segment 切 chunks（CJK 逐字 / ASCII 詞 / 空白）,chunks 帶 style;wrap 時按字型（bold/regular）的 widthOfTextAtSize 算寬;同行內合併連續同 style chunks 成 piece。

`applyCJKPunctuationRulesPieces` 跨 piece 邊界版:行首違規字符挪到上行末。同 style 合併成同 piece,不同 style 則插新 piece 保 style。

#### 17.12.5 cache 升級

W7 變動 plainText 結構（加 inline marker）→ cache key sha1 變 → 既有 PDF cache 全部失效。網頁 cache 跟 PDF cache 共用 `tc_<sha1>` 前綴（sha1 不可逆無法精準分）,W7 升級時一併清。

`background.js` 在 `onInstalled` + `onStartup` + SW 冷啟動三處呼叫 `cache.migrateClearTranslationCacheOnce(W7_CACHE_MIGRATION_FLAG)`,flag 防重跑（只清一次）。glossary cache 不在此清（術語表 prompt 沒變動）。

#### 17.12.6 已知限制

1. **LLM marker 對齊**:Gemini 對段落級 `⟦N⟧` 已穩,但跨字 inline marker 是更細任務;LLM 偶爾會交叉 nesting(`⟦b⟧⟦i⟧X⟦/b⟧⟦/i⟧`)→ parser fallback 為 plain regular,使用者看到無 inline 樣式但譯文仍正常。
2. **link 不在原 text run 上**:有些 PDF link 純靠 annotation rect 沒對應底層 textRun（掃描型 link / 圖片連結）→ run linkUrl 抽不到 → 譯文無視覺 underline。fallback 由 caller 把這些 link 用原 PDF rect 保留 annotation,點擊仍可用。
3. **中文 italic 視覺**:中文書法傳統不用 italic,12° skew 對中文字符視覺奇怪,跟 Word / CSS font-synthesis 行為一致。W8+ 可加「按 codepoint 切 piece,只 skew ASCII piece」。
4. **nested style + 跨 wrap 邊緣**:bold + italic + link 三層巢狀且譯文長到跨 wrap 行,當前 wrap 演算法可能在邊界產生小 piece。Acceptable trade-off;極端場景觀察使用者反饋再迭代。

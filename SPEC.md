...
- **Debug Bridge**：content.js 透過 CustomEvent 橋接，main world 可用 `shinkansen-debug-request` / `shinkansen-debug-response` 事件讀取 log（支援 `GET_LOGS`、`CLEAR_LOGS`、`GET_PERSISTED_LOGS`、`CLEAR_PERSISTED_LOGS`、`CLEAR_CACHE`、`TRANSLATE`、`RESTORE`、`GET_STATE`（回傳含 manifest `version`）、`GET_YT_DEBUG`、`CLEAR_RPD`、`GET_CACHE_STATS`（唯讀 cache 池快照）、`SET_GLOSSARY_ENABLED`（撥術語表一致化開關，唯一開放的寫入 action，供自動化測試））。僅限 Chromium：Firefox 的 main world 受 Xray 安全模型限制，讀不到 response 的 object detail，Debug Bridge 回讀不可用（消費者皆為 Chromium-only tooling，刻意不改協定）
